...
> 历史说明：本页记录 PR6 当时的独立调用协议和 Collection 级小样本诊断。PR7 已改为共享初始检索与判断的配对协议，并按所属论文限制 QASPER 检索；PR6 的真实数字不能继续用于判断改写收益。修正后的协议和负结果见 [PR7 自适应停止与配对评测](pr7-adaptive-research.md)。
