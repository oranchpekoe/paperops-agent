...
生产默认只执行初始检索；实验配置可显式增加最多两轮补检。每轮证据按文档与 Chunk 去重，并受单 Chunk 和总字符预算约束；模型判定充分但置信度低于阈值时仍视为证据不足。回答节点只接收充分度判断选出的相关证据；未知引用、重复引用或位置不明确的多个缺失标记都会失败关闭，只有单个有效引用漏标记时允许确定性补全。证据预算耗尽或改写后没有新增证据时不生成猜测答案。
...
QASPER 查询按所属论文限制检索。基线与 Agent 共享初始检索和充分度判断，只在初始证据不足时让 Agent 从 checkpoint 继续补检，以减少独立模型采样造成的伪差异。当前真实 3+3 诊断没有观察到改写收益，因此生产默认值为零改写，有界补检仅作为显式实验能力保留。实现、指标和负结果见 [PR7 自适应停止与配对评测](pr7-adaptive-research.md)。

## 13. PR8 多论文结构化比较

用户从同一 Collection 选择 2–8 篇已入库论文，并提供 1–6 个明确维度。系统先对完整论文 × 维度矩阵执行文档内检索和结构化抽取，再只对 `missing` 单元格执行至多一次聚焦补检。初始矩阵和最终矩阵同时持久化；跨论文引用、未知引用和不完整模型输出失败关闭，无新增证据或重复查询时提前停止。

验收条件：

- 每个请求的论文—维度组合恰好返回一个单元格；
- 支持结论必须包含能解析到所属论文 Chunk 的行内引用；
- 已支持单元格不会在补检轮次重复抽取；
